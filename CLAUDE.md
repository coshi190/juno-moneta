...
- The SDK is versioned and published (see `packages/sdk/package.json`); bump its version when making a released change to its public API. Publishing is manual: `npm publish` from `packages/sdk`, to the public npm registry. There is no publish workflow. `prepublishOnly` builds `dist` from the committed ABIs in `packages/sdk/src/abis`, so the codegen ordering rule above is the only thing keeping a published build off stale ABIs.
